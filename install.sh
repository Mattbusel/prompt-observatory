#!/bin/sh
# Install Prompt Observatory from the latest GitHub release.
#
#   curl -fsSL https://raw.githubusercontent.com/Mattbusel/prompt-observatory/master/install.sh | sh
#
...
# SHA256SUMS.txt, and copies one file (prompt-observatory) into INSTALL_DIR. Nothing else.
set -eu

REPO="Mattbusel/prompt-observatory"
PREFIX="prompt-observatory"      # archive name prefix
BIN="prompt-observatory"            # file name inside the archive
CMD="prompt-observatory"            # command name you will type
...
case ":$PATH:" in
  *":$INSTALL_DIR:"*) say "Try it: prompt-observatory" ;;
  *) say ""
     say "$INSTALL_DIR is not on your PATH yet. Add this line to ~/.bashrc or ~/.zshrc:"
     say "  export PATH=\"$INSTALL_DIR:\$PATH\""
     say "Then open a new terminal and run: prompt-observatory" ;;
esac
